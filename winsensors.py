"""
Experiment to attach to builtin sensors on windows
In particular, the accelerometer
"""
import winrt
from winrt.windows.devices.sensors import (
    Accelerometer,SensorAccessStatus,KnownSensor)


def getSensorData():
    """
    Get the data from the accelerometer sensor
    """
    from winrt.windows.devices.sensors import (
...
def listSensors():
    ...
    """
    sensors = [s.id
        for s in KnownSensor.get_sensor_for_type(Accelerometer.type_id)]
