# this is experimental to see if somebody has a directory locked and what process is the culprit
# see also:
...
class ProcessInfo:
    ...
    def _getProcessInfo(self):
        ...
        ftUser=FILETIME(0)
        if self.pid!=0:
            hProcess=win32api.OpenProcess(win32con.PROCESS_QUERY_LIMITED_INFORMATION,pywintypes.FALSE,self.pid)
            if hProcess:
                processStartTime=ctypes.c_uint(self.processStartTime)
                if win32api.GetProcessTimes(hProcess,ctypes.pointer(ftCreate),ctypes.pointer(ftExit),ctypes.pointer(ftKernel),ctypes.pointer(ftUser)) and \
                    win32api.CompareFileTime(ctypes.pointer(processStartTime),ctypes.pointer(ftCreate)) == 0:
                    imageName=ctypes.c_wchar*win32con.MAX_PATH
                    imageNameLen=ctypes.c_uint(win32con.MAX_PATH)
                    if win32api.QueryFullProcessImageNameW(hProcess,0,imageName,ctypes.pointer(imageNameLen)) and imageNameLen<=win32con.MAX_PATH:
...
def processLockingFile(
    ...
    :recursive: if filename is a directory, keep going with subdirectories and files
    ...
    #print(f'RmStartSession key={szSessionKey.value} session={dwSession.value}')
    if dwError!=0: # success
        raise Exception('[Windows error 0x%02X] %s'%(dwError,win32api.FormatMessage(dwError)))
    pszFile=ctypes.c_wchar_p(filename)
    numFiles=1
    dwError=rstrtmgr.RmRegisterResources(dwSession,numFiles,ctypes.byref(pszFile),0,None,0,None)
    ...
    dwError=rstrtmgr.RmGetList(dwSession,ctypes.byref(nProcInfoNeeded),ctypes.byref(nProcInfo),pRgpi,ctypes.byref(dwReason))
    if dwError==5:
        # ACCESS_DENIED - sometimes trying again helps
        dwError=rstrtmgr.RmGetList(dwSession,ctypes.byref(nProcInfoNeeded),ctypes.byref(nProcInfo),pRgpi,ctypes.byref(dwReason))
    if dwError!=0: # https://docs.microsoft.com/en-us/windows/win32/debug/system-error-codes--0-499-
        raise Exception('[Windows error 0x%02X] %s'%(dwError,win32api.FormatMessage(dwError)))
    #print(f'RmGetList returned {nProcInfo.value} infos ({nProcInfoNeeded} needed)')
    for i in range(nProcInfo.value):
        pi=ProcessInfo(rgpi[i].strAppName,rgpi[i].Process.dwProcessId,rgpi[i].ApplicationType)
    ...
    if recursive and os.path.isdir(filename):
        for f in os.listdir(filename):
            yield from processLockingFile(os.sep.join((filename,f)),recursive,ignore,True)
...
def cmdline(args:typing.Iterable[str])->int:
    ...
    if printhelp or not didSomething:
        ...
        print('OPTIONS:')
        print('  -r ............................. recursive (keep going till you find one)')
        print('  -h ............................. this help')
        return 1
    return 0
